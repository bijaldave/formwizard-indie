...
import { percentageToPoints, formatIndianNumber } from './fieldMaps';
import { templateManager } from './templateManager';
import { renderDebugOverlay } from './debug';

// Form 15H specific anchors (to be calibrated with actual template)
export const CANONICAL_15H_ANCHORS: { [key: string]: { x: number; y: number } } = {
  "name of assessee": { x: 120.5, y: 750.2 },
  "pan of the assessee": { x: 450.8, y: 750.2 }, 
  "signature of the declarant": { x: 150.3, y: 180.5 }
};

// Form 15H field mappings (similar to 15G but may have different positions)
export const FORM_15H_FIELDS: { [key: string]: { x: number; y: number; width: number; height: number } } = {
...
export function profileToForm15HData(profile: Profile, dividend: DividendRow): Form15HData {
  ...
  return {
    ...
    income_for_decl: dividend.total.toFixed(2),
    ...
    section: "194",
    dividend_amount: dividend.total.toFixed(2),
...
/**
 * Fill Form 15H PDF with user data using anchor-based positioning
 */
export async function fillForm15H(
  ...
  if (cachedTemplate?.affineMatrix) {
    ...
    for (const anchor of detectedAnchors) {
      const canonicalPoint = CANONICAL_15H_ANCHORS[anchor.text];
      if (canonicalPoint) {
  ...
  // Fill all fields
  await fillAll15HFields(firstPage, font, data, affineMatrix);
  ...
  if (debugMode && cachedTemplate) {
    await renderDebugOverlay(
      ...
      Object.values(CANONICAL_15H_ANCHORS)
    );
  }
...
/**
 * Fill all Form 15H fields with transformed coordinates
 */
async function fillAll15HFields(
  page: PDFPage, 
  font: any, 
  data: Form15HData, 
  ...
  const drawTextField = (fieldName: string, text: string, rightAlign: boolean = false) => {
    ...
    const field = FORM_15H_FIELDS[fieldName];
    if (!field) return;
  ...
  if (data.signature) {
    try {
      ...
      const field = FORM_15H_FIELDS['signature'];
      const canonicalRect = percentageToPoints(field);
      const transformedRect = transformRectangle(affineMatrix, canonicalRect);
